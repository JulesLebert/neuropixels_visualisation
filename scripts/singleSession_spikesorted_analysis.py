### Trying to hack the concatenated analysis into something that allows single session analysis. Should be far easier as it will not require complicated timing links, but I will need to do it basically from scratch... Alternately, I can rerun spikesorting with a concatenation of the two pure tone sessions I did... This feels like a bad long term plan with not much short term gain.
...
class concatenatedAnalysis(concatenatedNeuralData):
    ...
    def testFun_summary_pdf(self, units, title, savedir): # made seperate so that I can adjust stuff independently
        events_args = {
                       ...
                       1: {'Trial Start': ['No Level Cue', 'Sound Right'],
                           'ax_to_plot': ['B', 'E']},
                       # 3: {'Trial Start': ['No Level Cue', 'Sound Right'],
                       #     'ax_to_plot': ['C', 'F']},
                       #
        ...
        # cluster_ids = units
        windowToCheck = [-1, 2]
        self.testFun_summary_figure(units, events_args,window=windowToCheck) # "units" inputted instead of cluster, because this code I am currently working on wants all units for one trial.
        ...
        #         #pdf.savefig(fig)
                #plt.close(fig)
    def testFun_summary_figure(self, cluster_ids, events_args, window=[-1, 2]):

        for i, (fig_num, params) in enumerate(events_args.items()):
            evtype = list(params.keys())[0]
            filters = params[evtype]
            all_filtered_bhv = self.gather_sounds_per_trial(filters) # has sound files per trial, all sessions.
            clustersDone = 0
            ...
            for sessionIter, sessionNum in enumerate(np.unique(aligned_spikes[:,3])):
                ...
                for trialNumIter, trialNum in enumerate(np.unique(aligned_spikes[aligned_spikes[:,3]==sessionNum,0])):
                    ...
                    spikeTrains = np.zeros((int(np_fs) * (window[1] - window[0]), len(cluster_ids)))  ### one train per trial, including the excluded at the moment.
                    filtedTrains = np.zeros((int(np_fs) * (window[1] - window[0]), len(cluster_ids)))  # make another zeros array, same size
                    ...
                    timeFrame = np.linspace(window[0], window[1], (int(np_fs) * (window[1] - window[0])))
                    ...
                    for cluster_id in cluster_ids:
                        ...
                        for spikeTime in aligned_spikes_t_and_c:
                            ...
                            spikeTrains[index_min,cluster_id] += 1
                        filtedTrains[:,cluster_id] = cpx_sig.filtfilt(gaussianKernel,1,spikeTrains[:,cluster_id]).get()
                    ...
                    testSpectrogram = resample(testSpectrogram, (analysis_fs)*(window[1]-window[0])) ### resamples to the number of samples said in second argument... By subtracting windows, we get correct length.
                    resampled_filtered_spikes = resample(filtedTrains, (analysis_fs)*(window[1]-window[0]))
...
def run_concatenated():
    ...
    dataset.load()
    dataset.create_summary_pdf(saveDir, title=analysisName,doUnsorted=True)

def run_single_session_analysis():
    filter_trials = {'No Level Cue'}
    ...
    dp = Path('D:/Jeffrey/Output/Tones/spikesorted/F2302_Challah/06122024_AM_Challah_g0/KiloSortSortingExtractor/KiloSortSortingExtractor/')
    phyFolder = dp / 'phy_folder'
    sortingsFolder = dp / 'sortings_folder'
    ...
    ### old way
        ...
        #dataset.load()
    ### new way, where I just load the sortings struct.
    if (sortingsFolder).exists():  ### This seems like an odd bit of code, I should check the original
        we = si.load_sorting_analyzer(sortingsFolder)
